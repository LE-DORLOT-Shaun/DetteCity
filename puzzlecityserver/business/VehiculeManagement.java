...
public class VehiculeManagement {
	...
	public VehiculeManagement() throws SQLException, InterruptedException {
//		this.c = c;
		Connection c = ConnectionPool.getConnection();
		if (c != null) {
			System.out.println("Connexion cree");
			try {
				totalVehicule = getVehicule(c);
				threshold = vehiculeThreshold(c);
				alertP = getPollutionAlert(c);
				ConnectionPool.releaseConnection(c);
			} catch (SQLException | InterruptedException e) {
				e.printStackTrace();
			}

			System.out.println("Fin VM et co rendu");
		} else {
...
*/
	/*
	 * method to get the actual number of cars in town it used in the beginning and
	 * the data are used by the Thread server and sent to the Client
	 */
	public int getVehicule(Connection c) throws SQLException, InterruptedException {
	...
	/*
	 * method to get the max number of cars in town it used in the beginning and the
	 * data are used by the Thread server and sent to the Client
	 */

	public int vehiculeThreshold(Connection c) throws SQLException, InterruptedException {
	...
	public JSONObject updateMaxCars(int maxCars) throws SQLException, InterruptedException {
		...
		PreparedStatement stmt = c
				.prepareStatement("update threshold set vehicule_threshold= ?, last_update= current_timestamp;");
	...
	public Object addCarToHistory(String vehicule, String type, int sensorId) throws SQLException, InterruptedException {
		...
		this.adjustCarsNb(c, vehicule, type);
		System.out.println(obj);
		ConnectionPool.releaseConnection(c);
		return obj;
	}

	public void adjustCarsNb(Connection c, String vehicule, String type) throws SQLException, InterruptedException {
		PreparedStatement stmtGetInfos = c.prepareStatement(
				"SELECT sensor_id FROM vehicule_history where brand=? AND type=? ORDER BY schedule DESC LIMIT 1;");
		...
		int currentNbCars = this.getVehicule(c);
		JSONObject obj = new JSONObject();

		if (sensorType.equals("IN")) {
			currentNbCars++;
			PreparedStatement stmt = c
					.prepareStatement("insert into traffic(vehicule_nb, date) values (?,CURRENT_TIMESTAMP);");
	...
	public Object SearchCars(Connection c, String dateDebut, String dateFin, String zone, String type)
	...
	public Object PollutionAlert(Connection c) throws SQLException, InterruptedException {
	...
	public boolean getPollutionAlert(Connection c) throws SQLException, InterruptedException {
